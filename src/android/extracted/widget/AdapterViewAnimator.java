...
public abstract class AdapterViewAnimator extends AdapterView<Adapter>
     ...
     */
    private final int mRestoreWhichChild = -1;
    ...
    public AdapterViewAnimator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //
        // resource =
        // a.getResourceId(com.android.internal.R.styleable.AdapterViewAnimator_outAnimation, 0);
        // if (resource > 0) {
    ...
    private void setDisplayedChild(int whichChild, boolean animate) {
        if (mAdapter != null) {
            ...
            final boolean hasFocus = getFocusedChild() != null;
    ...
    View getViewAtRelativeIndex(int relativeIndex) {
        if (relativeIndex >= 0 && relativeIndex <= getNumActiveViews() - 1 && mAdapter != null) {
            final int i = modulo(mCurrentWindowStartUnbounded + relativeIndex, getWindowSize());
    ...
    private ViewAndMetaData getMetaDataForChild(View child) {
        for (final ViewAndMetaData vm: mViewsMap.values()) {
            if (vm.view == child) {
                return vm;
            }
        }
    ...
    LayoutParams createOrReuseLayoutParams(View v) {
        ...
        if (currentLp instanceof ViewGroup.LayoutParams) {
            final LayoutParams lp = currentLp;
            return lp;
        }
    ...
    void refreshChildren() {
        ...
        for (int i = mCurrentWindowStart; i <= mCurrentWindowEnd; i++) {
            final int index = modulo(i, getWindowSize());
    ...
    void showOnly(int childIndex, boolean animate) {
        ...
        for (int i = 0; i < mPreviousViews.size(); i++) {
            final View viewToRemove = mViewsMap.get(mPreviousViews.get(i)).view;
            mViewsMap.remove(mPreviousViews.get(i));
            viewToRemove.clearAnimation();
            if (viewToRemove instanceof ViewGroup) {
                final ViewGroup vg = (ViewGroup) viewToRemove;
        ...
        mPreviousViews.clear();
        final int newWindowStartUnbounded = childIndex - mActiveOffset;
        final int newWindowEndUnbounded = newWindowStartUnbounded + getNumActiveViews() - 1;
        ...
        final int rangeStart = modulo(newWindowStart, getWindowSize());
        final int rangeEnd = modulo(newWindowEnd, getWindowSize());
        ...
        for (final Integer index : mViewsMap.keySet()) {
            ...
            if (remove) {
                final View previousView = mViewsMap.get(index).view;
                final int oldRelativeIndex = mViewsMap.get(index).relativeIndex;
        ...
        if (!(newWindowStart == mCurrentWindowStart && newWindowEnd == mCurrentWindowEnd &&
            ...
            for (int i = newWindowStart; i <= newWindowEnd; i++) {
                ...
                final int newRelativeIndex = i - newWindowStartUnbounded;
                ...
                final boolean inOldRange = mViewsMap.containsKey(index) && !mPreviousViews.contains(index);

                if (inOldRange) {
                    final View view = mViewsMap.get(index).view;
                ...
                // Otherwise this view is new to the window
                    ...
                    final View newView = mAdapter.getView(adapterPosition, null, this);
                    final long itemId = mAdapter.getItemId(adapterPosition);
                    ...
                    final FrameLayout fl = getFrameForChild();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final boolean haveChildRefSize = (mReferenceChildWidth != -1 && mReferenceChildHeight != -1);
        ...
        if (heightSpecMode == MeasureSpec.UNSPECIFIED) {
            ...
            if (haveChildRefSize) {
                final int height = mReferenceChildHeight + getPaddingTop() + getPaddingBottom();
        ...
        if (widthSpecMode == MeasureSpec.UNSPECIFIED) {
            ...
            if (haveChildRefSize) {
                final int width = mReferenceChildWidth + getPaddingLeft() + getPaddingRight();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final int childRight = getPaddingLeft() + child.getMeasuredWidth();
            final int childBottom = getPaddingTop() + child.getMeasuredHeight();
